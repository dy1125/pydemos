...
def getContent(mainContent):
    ...
    date = soup.select(".sou text_dot_line")
    if  pss and date:
        print("==类型:  .show_text   .sou text_dot_line")
        return pss,date

    soup = BeautifulSoup(mainContent,"html.parser")
   pss = soup.select(".w1000 txtCon clearfix")
    date = soup.select(".editor timel clearfix")
    if  pss and date:
        print("==类型:  .w1000 txtCon clearfix  .editor timel clearfix")
        return pss,date

    #网址  http://chuxin.people.cn/n1/2019/1127/c428144-31476915.html
   ...
   pss = soup.select(".show_text")
    ...
    if  pss and date:
        print("==类型:  .show_text   #p_publishtime")
        return pss,date
...
def article(art_url):
    ...
    with open("人民网总报道量.txt","a+",encoding="utf-8") as file:     #####改名字：一个网站加一个词
	    file.write(str(pss)+"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")    


for page_number in range(1,5):   #所有负面词列表页999页   #####改页数：人民网共13570篇，每页20篇
