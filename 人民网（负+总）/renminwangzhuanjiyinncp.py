...
def getContent(mainContent):
   ...
   pss = soup.select("div #rwb_zw")
    ...
    soup = BeautifulSoup(mainContent,"html.parser")
    pss = soup.select(".shou_text")
    date = soup.select(".sou")
    if  pss and date:
        print("==类型:  .shou_text  .sou")
        return pss,date
  ...
  soup = BeautifulSoup(mainContent,"html.parser")
   ...
   pss = soup.select("div #p_content")
    date = soup.select("span #p_publishtime")
    if  pss and date:
        print("==类型:  div #rwb_zw  .f1")
        return pss,date
       # 该文章网址  http://env.people.com.cn/n1/2016/0414/c1010-28276720.html
   ...
   pss = soup.select("div #p_content")
    ...
    if  pss and date:
        print("==类型:  div #rwb_zw  .f1")
        return pss,date
     #该文章网址  http://finance.people.com.cn/n1/2016/0414/c1004-28274501.html   
...
def article(art_url):
    ...
    with open("人民网转基因农产品.txt","a+",encoding="utf-8") as file:     #####改名字：一个网站加一个词
	    file.write(str(pss)+"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")    


for page_number in range(1,5):   #所有负面词列表页999页   #####改页数：人民网共853篇，每页20篇
