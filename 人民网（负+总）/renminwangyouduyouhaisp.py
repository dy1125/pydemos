...
def getContent(mainContent):
    ...
    pss = soup.select(".w1000 texyCon clearfix")
    date = soup.select(".editor timel cleaifix")
    if  pss and date:
        print("==类型:  .w1000 texyCon clearfix   .editor timel cleaifix")
        return pss,date
#网址  http://chuxin.people.cn/n1/2019/1127/c428144-31476717.html  
    soup = BeautifulSoup(mainContent,"html.parser")
   ...
   pss = soup.select(".show_text")
    ...
    if  pss and date:
     ...
     soup = BeautifulSoup(mainContent,"html.parser")
   pss = soup.select(".p_conent")
    date = soup.select("#p_publishtime")
    if  pss and date:
        print("==类型:  .p_content  #p_publishtime")
        return pss,date
   ...
   pss = soup.select(".p_conent")
    ...
    soup = BeautifulSoup(mainContent,"html.parser")
   pss = soup.select("div #p_content")
    date = soup.select(".sou")
    if  pss and date:
        print("==类型:  div #p_content  .sou")
        return pss,date
...
def article(art_url):
    ...
    with open("人民网有毒有害食品.txt","a+",encoding="utf-8") as file:     #####改名字：一个网站加一个词
	    file.write(str(pss)+"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")    


for page_number in range(1,5):   #所有负面词列表页999页   #####改页数：人民网共3327篇，每页20篇
